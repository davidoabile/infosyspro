...
namespace Infosyspro\Mobile ;

/**
 * This class handles all mobile detecttion using php get_browser()
 * 
 */
class Mobile
{
	...
	//
	foreach ( $this->tabletDevices as $name => $_regex ) {
	    if ( preg_match( '/' . $_regex . '/is', $this->browser[ 'browser_name_pattern' ] ) ) {
		$this->isTablet = true ;
		$this->devicename = $name ;
	    }
	}
	//Get device name
	foreach ( $this->phoneDevices as $name => $_regex ) {
	    if ( preg_match( '/' . $_regex . '/is', $this->browser[ 'browser_name_pattern' ] ) ) {
		$this->devicename = $name ;
	    }
	}
    ...
    /**
     * Returs the device name 
     * only handheld devices will be returned
     * 
     * @return string
     */
    ...
    /**
     * Gets the platform running the user agent
     * 
     * @return string 
     */
    ...
    /**
     * Check if the UA is a mobile device
     * 
     * @return boolean
     */
    public function isMobileDevice ()
    {
	return ( bool ) $this->browser[ 'ismobiledevice' ] ;
    }
    ...
    {
	return $this->browser[ 'platform_version' ] ;
    }
     ...
     * @return bool
     */
    public function supportJs ()
    {
	return $this->browser[ 'javascript' ] ;
    }

    /**
     * Check if the requester is a crawler
     * 
     * @return bool
     */
    ...
    /**
     * Checks if this is a reader
     * 
     * @return boolean
     */
    public function isSyndicateReader ()
    {
